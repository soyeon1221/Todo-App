...
- trim 메서드를 사용해서 input이 비어있을 때 추가버튼이 비활성화되도록 구현하였습니다.
- input을 저장하면 체크박스, 저장한 이름, 수정버튼, 삭제버튼이 나타나도록 구현하였습니다.
- saveDoc 함수를 사용해서 로컬 스토리지에 동기화하여 새로 고침을 방지할 수 있도록 구현하였습니다.
...
- trim 메서드를 사용해서 input에 내용을 입력했을 때만 저장버튼이 활성화되도록 구현하였습니다.
...
- 조건부 연산자를 사용하여 버튼을 눌렀을 때 filter 상태에 따라 다른 메시지를 반환합니다. 
- 논리연산자를 사용해서 클릭한 해당 버튼 색만 어둡게 적용하였습니다.
